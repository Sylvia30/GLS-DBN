...
% vol. 47, no. 9, pp. 3179?191, Sep. 2014.
        ...
        function [deltaWeightReg,deltaVisBiasReg,deltaHidBiasReg]=getRegularizationGradient(obj,batchData,posHid)
            switch obj.rbmParams.sparsityMethod
                ...
                case 'Laplace'
                    ...
                    for i = 1:500
                        if term1(1,i) < p
                            term3 = 0.5*exp(-(p-mean(posHid(:,i))));
                        else
                            term3 = -0.5*exp(-(mean(posHid(:,i))-p));
                        end  
                    end
                    
                    deltaHidBiasReg = term3 * term2;
                    deltaWeightReg = (1/size(batchData,1))*(batchData'*term3 * term2) 
