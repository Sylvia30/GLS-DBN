    ...
    methods (Access=public)
        ...
        function train(obj,data)
            ...
            for epoch=1:obj.rbmParams.maxEpoch
                ...
                %计算稀疏度
                actPro =  1./(1+exp(-(batchData*weight + repmat(hidBias,size(batchData,1),1))));
                sparseness = mean((repmat(sqrt(500),size(batchData,1),1)-(sum(abs(actPro)'))'./(sqrt(sum((actPro.^2)')')))/(sqrt(500)-1));
                fprintf(num2str(sparseness),'\n');
                r(1,epoch) = sparseness;
